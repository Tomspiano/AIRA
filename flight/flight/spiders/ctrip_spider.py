...
class CtripSpider(scrapy.Spider, ABC):
    ...
    def get_info(self, quest):
        self.dcityname = quest['dcityname']
        self.dcity = quest['dcity']
        self.acities = quest['acities']
        self.date = quest['date']

    def start_requests(self):
        for acityname, acity in self.acities.items():
            token = self.context.call('get_token', self.dcity, acity, 'Oneway')
            headers = {
